...
namespace Leap.Unity {
  ...
  public class CustomCapsuleHand : HandModelBase {
    ...
    [SerializeField]
    private Material _virtualMaterial;
    ...
    [SerializeField]
    private GameObject _handModelRelativeTransform;
    ...
    public override void UpdateHand() {
      ...
      // Draw Spheres
      Graphics.DrawMeshInstanced(_sphereMesh, 0, _realMaterial, _sphereMatrices, _curSphereIndex, null, 
      ...
      Graphics.DrawMeshInstanced(_cylinderMesh, 0, _realMaterial, _cylinderMatrices, _curCylinderIndex, null,
        _castShadows ? UnityEngine.Rendering.ShadowCastingMode.On : UnityEngine.Rendering.ShadowCastingMode.Off, true, gameObject.layer);

      Matrix4x4 relativeTransform = Matrix4x4.TRS(_handModelRelativeTransform.transform.position, _handModelRelativeTransform.transform.rotation, new Vector3(1, 1, 1));
      Matrix4x4[] _sphereMatrices2 = new Matrix4x4[_sphereMatrices.Length];
      Matrix4x4[] _cylinderMatrices2 = new Matrix4x4[_cylinderMatrices.Length];
      for (int i = 0; i < _sphereMatrices2.Length; i++) {
        _sphereMatrices2[i] = relativeTransform * _sphereMatrices[i];
      }
      for (int i = 0; i < _cylinderMatrices2.Length; i++) {
        _cylinderMatrices2[i] = relativeTransform * _cylinderMatrices[i];
      }

      // Draw 2
      Graphics.DrawMeshInstanced(_sphereMesh, 0, _virtualMaterial, _sphereMatrices2, _curSphereIndex, null, 
        _castShadows?UnityEngine.Rendering.ShadowCastingMode.On: UnityEngine.Rendering.ShadowCastingMode.Off, true, gameObject.layer);

      Graphics.DrawMeshInstanced(_cylinderMesh, 0, _virtualMaterial, _cylinderMatrices2, _curCylinderIndex, null,
        _castShadows ? UnityEngine.Rendering.ShadowCastingMode.On : UnityEngine.Rendering.ShadowCastingMode.Off, true, gameObject.layer);
    }
